...
class InceptionBlock(nn.Module):
        ...
        for i in range(groups):
            self.channel_attn.append(
                nn.Sequential(
                    ...
                    linear(c_group,
                           c_reduction),
                    nn.BatchNorm1d(c_reduction),
                    nn.ReLU(inplace=True),
                    linear(c_reduction,
                           ch_out)))
            
            ops = [conv2d(c_group, c_reduction, kernel_size=1),
            ...
            ops.extend([
                conv2d(c_reduction, 1, kernel_size=1),
                # nn.Sigmoid(),
                # Interpolate(size=(sz_out, sz_out), mode='nearest')
            ])
    ...
    def forward(self, x_small, x_big):
        ...
        assert x_small.size(2) == self.sz_in and x_big.size(2) == self.sz_out
        ...
        sp_outs = torch.cat(sp_outs, dim=1)
        
        sp_outs = self.noise_injection(sp_outs)
        
        sp_outs = F.sigmoid(sp_outs)
        
        sp_outs = F.interpolate(sp_outs, self.sz_out)
        
        sp_outs = sp_outs.unsqueeze(2)
...
class Generator(nn.Module):
    def __init__(self, ngf=64, nz=100, nc=3, im_size=1024, groups=4, reduction=16):
        ...
        self.se_128 = InceptionBlock(nfc[8],
                                    8,
                                    nfc[128],
                                    128,
                                    groups=groups,
        ...
        self.se_256 = InceptionBlock(nfc[16],
                                    16,
                                    nfc[256],
                                    256,
                                    groups=groups,
        ...
        if im_size > 256:
            ...
            self.se_512 = InceptionBlock(nfc[32],
                                    32,
                                    nfc[512],
                                    512,
                                    groups=groups,
